...
void MN_LoadSlotText(void)
{
    ...
    {
#ifndef OF_PC
        // Pocket saves carry a wrapper header; read the description through the
        // slot reader so the menu shows the real name, not the header bytes.
        byte header[SAVESTRINGSIZE + 8];

        if (!I_OpenFPGASaveReadHeader(P_SaveGameFile(i), header, sizeof(header)))
        {
            SlotText[i][0] = 0;
            SlotStatus[i] = 0;
            continue;
        }
        memcpy(SlotText[i], header, SLOTTEXTLEN);
        SlotText[i][SLOTTEXTLEN] = '\0';
        SlotStatus[i] = 1;
#else
        int retval;
...
static void DrawOptionsMenu(void)
{
    ...
    if (swap_run_walk)
    {
        MN_DrTextB(DEH_String("RUN"), 196, 110);
    }
    else
    {
...
static void DrawOptions2Menu(void)
{
    ...
    MN_DrTextB(DEH_String(control_scheme == CONTROL_SCHEME_DISCO
                          ? "DISCO" : "DEFAULT"), 196, 140);
}
