...
// Hexen translation unit that includes of_gpu.h.  Ported from the Doom
...
{
    /* The GPU blend LUT is indexed [shaded_src][fb_byte] - the SAME
     * orientation Hexen's R_DrawTLColumn uses (tinttable[dst + (src<<8)]),
     * so TINTTAB uploads as-is.  R_DrawAltTLColumn and the Translated*
     * variants index dst-major and stay on the CPU.  The RTL quantises
     * the source axis to 7 bits, so TL blends are sub-JND approximate. */
    if (tinttable == NULL)
        return;

    memcpy(gpu_fuzz_transluc_table, tinttable,
