...
#include "p_local.h"
#ifndef OF_PC
#include "p_saveg.h"   // Pocket NVRAM save layer (in-RAM VFS + LZSS slots)
#endif
...
#ifndef OF_PC
    // Pocket: persist the slot's VFS files to one LZSS-compressed NVRAM slot.
    OFX_FlushSlot(slot);
#endif
}
...
void SV_LoadGame(int slot)
{
    ...
    {
#ifndef OF_PC
        // Pocket: decompress the slot from NVRAM into the VFS first. The
        // reborn slot has no NVRAM file and is left untouched (it lives in RAM).
        OFX_EnsureSlot(slot);
#endif
...
{
#ifndef OF_PC
    // Pocket: copy within the in-RAM VFS (no host filesystem).
    OFX_Copy(source_name, dest_name);
    return;
#else
...
{
#ifndef OF_PC
    // Pocket: read from the in-RAM VFS (loaded from NVRAM by OFX_EnsureSlot).
    SavingFP = OFX_OpenRead(fileName);
#else
...
{
#ifndef OF_PC
    // Pocket: capture the write into the in-RAM VFS; SV_Close commits it.
    SavingFP = OFX_OpenWrite(fileName);
#else
...
static void SV_Close(void)
{
    ...
    {
#ifndef OF_PC
        // Pocket: a finished write lives in the scratch buffer; commit it to
        // the in-RAM VFS entry before closing.
        OFX_BeforeClose(SavingFP);
#endif
        fclose(SavingFP);
        SavingFP = NULL;
    }
