...
#include "m_menu.h"
#ifndef OF_PC
#include "p_saveg.h"   // OFX_SlotDescription: read slot names from NVRAM
#endif
...
{
#ifndef OF_PC
    // Pocket: the slot description lives in the NVRAM .sav wrapper header.
    return OFX_SlotDescription(slot, description);
#else
    FILE *fp;
    boolean found;
    char name[100];
...
static void DrawOptionsMenu(void)
{
    // Values sit at x=210 so they clear the widest label ("MOVEMENT : ").
...
void MN_DeactivateMenu(void)
{
    ...
    S_StartSound(NULL, SFX_PLATFORM_STOP);
    P_ClearMessage(&players[consoleplayer]);
    SB_state = -1;   // repaint the status bar to clear any menu overdraw
}
