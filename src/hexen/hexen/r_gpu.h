/* r_gpu.h — openfpgaOS Hexen stub (not upstream chocolate-doom).
 *
 * The openfpgaOS port accelerates Doom's column/span drawing on the FPGA
 * through R_GPU_* (cdoom/doom/r_gpu.c). Hexen runs the stock software
 ...
 * Hexen's own renderer never references r_gpu. Only the shared/shim layer
 * includes this header when building the Hexen core. */
#ifndef OF_HEXEN_R_GPU_STUB_H
#define OF_HEXEN_R_GPU_STUB_H
