...
#ifndef OF_PC
// openfpgaOS Pocket save integration (no upstream counterpart). Slots are named
// "<prefix>_<slot>.sav" (prefix from -saveprefix); SV_Filename and P_SaveGameFile
// build the same basename so file I/O and the shim's slot layer agree.
#define OPENFPGA_SAVE_PREFIX_MAX 48
static char openfpga_save_prefix[OPENFPGA_SAVE_PREFIX_MAX] = "heretic";
...
// Pocket slot basename for a save slot, e.g. "heretic_3.sav".
char *P_SaveGameFile(int slot)
{
    ...
    {
        M_snprintf(openfpga_save_filenames[slot],
...
// In-RAM save transport for the Pocket (no host filesystem). SV_* read/write
// through an fmemopen FILE* over this buffer; SV_Close flushes to NVRAM
// (I_OpenFPGASaveWrite) and SV_OpenRead pre-loads it (I_OpenFPGASaveRead).
#define OPENFPGA_SAVE_BUFFER_SIZE 0x40000
static byte openfpga_save_buffer[OPENFPGA_SAVE_BUFFER_SIZE];
static char openfpga_active_save[64];
static boolean openfpga_save_writing;

// Derive the slot identity from the filename Heretic built (SV_Filename), since
// savegameslot is set only when saving, not loading.
static const char *OpenFPGA_SaveBasename(const char *path)
{
...
#ifndef OF_PC
    // Match the Pocket save basename so Heretic's file I/O and the shim's
    // slot layer (P_SaveGameFile) reference the same "<prefix>_<slot>.sav".
    filename_len = strlen(savegamedir) + OPENFPGA_SAVE_PREFIX_MAX + 16;
    filename = malloc(filename_len);
    M_snprintf(filename, filename_len,
               "%s%s_%d.sav", savegamedir, openfpga_save_prefix, slot);
#else
...
{
#ifndef OF_PC
    // Pocket: capture the save into an in-RAM buffer; SV_Close flushes it.
    M_StringCopy(openfpga_active_save, OpenFPGA_SaveBasename(fileName),
                 sizeof(openfpga_active_save));
    openfpga_save_writing = true;
    SaveGameFP = fmemopen(openfpga_save_buffer, sizeof(openfpga_save_buffer),
                          "wb");
#else
...
{
#ifndef OF_PC
    // Pocket: pull the slot out of NVRAM into the buffer, then read from it.
    size_t length = 0;

    openfpga_save_writing = false;
    if (!I_OpenFPGASaveRead(OpenFPGA_SaveBasename(filename),
                            openfpga_save_buffer, sizeof(openfpga_save_buffer),
                            &length))
    {
        I_Error("Could not load savegame %s", filename);
    }
    SaveGameFP = fmemopen(openfpga_save_buffer, length, "rb");
#else
...
void SV_Close(void)
{
    ...
    {
#ifndef OF_PC
        // Pocket: a finished write lives in the RAM buffer; persist it to the
        // slot's NVRAM through the shim's wrapped-save writer before closing.
        if (openfpga_save_writing)
        {
            long written;
